...
const setupFloatingLanguageSwitch = () => {
  const source = document.querySelector(".site-header .lang-switch");
  if (!source || document.querySelector(".lang-switch-clone")) {
    return;
  }

  const clone = source.cloneNode(true);
  clone.classList.add("lang-switch-clone");
  document.body.appendChild(clone);

  const positionClone = () => {
    const rect = source.getBoundingClientRect();
    clone.style.left = "auto";
    clone.style.right = "clamp(22px, 4vw, 68px)";
  ...
  window.addEventListener("scroll", positionClone, { passive: true });
};
